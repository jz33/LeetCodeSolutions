...
that is less than or equal to A[1].  A[3] is also the largest value, but 2 is a smaller index, so we can only jump to i = 2 and
not i = 3.
...
'''
class Solution:
    def getJumps(self, indexes: List[int]):
        '''
        @indexes: indexes of the array when array is sorted
        '''
        jumps = [None] * len(indexes)
        stack = [] # The stack contains decsending indexes
        for i in indexes:
            while stack and i > stack[-1]:
                j = stack.pop()
                # i > j, there is a jump.
                # For odd jump, indexes are sorted ascendingly,
                # then A[i] >= A[j]
                # For even jump, indexes are sorted descendingly,
                # then A[i] <= A[j]
                jumps[j] = i
            stack.append(i)
        return jumps
    ...
    def oddEvenJumps(self, A: List[int]) -> int:
        ...
        # Build 2 maps of next available jumps for odd and even move
        # Notice 2nd cannot just use reversed iteration from 1st sorted array,
        # because Python sort is stable. For example, an array with duplicates,
        # [2,3,1,1,4], sort indexes: [2,3,0,1,4], sort reversely: [4,1,0,2,3],
        # not [4,1,0,3,2], and sort reversely is what we need here, because
        # the smaller index is preferred.
        oddyNext = self.getJumps(sorted(range(N), key = lambda x : A[x]))
        evenNext = self.getJumps(sorted(range(N), key = lambda x : -A[x]))

        # Compute the reachable points from back.
        # oddyReachable[i] means whether at i, odd jump, it is reachable to final.
        oddyReachable = [False] * (N-1) + [True]
        evenReachable = [False] * (N-1) + [True]

        for i in range(N-2, -1, -1):
            if oddyNext[i] is not None:
                # If at i, odd move, if oddyNext is not None,
                # then there is a jump from i to oddNext[i].
                # If evenReachable[oddyNext[i]] can reach the final, then
                # oddyReachable[i] can reach too
                oddyReachable[i] = evenReachable[oddyNext[i]]
            if evenNext[i] is not None:
                evenReachable[i] = oddyReachable[evenNext[i]]

        # Only count oddyReachable because starting jump is odd
        return sum(i is True for i in oddyReachable)
