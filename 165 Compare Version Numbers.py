...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        v1 = list(int(x) for x in version1.split("."))
        v2 = list(int(x) for x in version2.split("."))
        ...
        for i in range(min(len(v1), len(v2))):
            ...
            # Notice here, i is min(len(v1), len(v2)) - 1
            i += 1
            if len(v1) > len(v2):
                if all(n == 0 for n in v1[i:]):
                    return 0
                return 1
            elif len(v1) < len(v2):
                if all(n == 0 for n in v2[i:]):
                    return 0
                return -1
            else:
                return 0
