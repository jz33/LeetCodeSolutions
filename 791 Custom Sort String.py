...
class Solution:
    def customSortString(self, order: str, target: str) -> str:
        targetCounter = collections.Counter(target) # {char: appearance count}

        result = [] # ['aa', 'bb', ...]
        for char in order:
            count = targetCounter.get(char)
            if count:
                result.append(''.join([char] * count))
                del targetCounter[char]

        for char, count in targetCounter.items():
            if count:
                result.append(''.join([char] * count))
