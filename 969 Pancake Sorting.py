...
'''
class Solution:
    def flip(self, A: List[int], count: int) -> List[int]:
        return A[:count][::-1] + A[count:]
    
    def pancakeSort(self, A: List[int]) -> List[int]:
        '''
        Find largest element, then do 2 flips to put to end,
        need 2 * (len(A) - 1) flips
        '''
        flips = []
        
        for r in range(len(A), 1, -1):
            maxIndex = max(range(r), key = lambda i : A[i])
            flips += [maxIndex + 1, r]
            
            A = self.flip(A, maxIndex + 1)
            A = self.flip(A, r)
