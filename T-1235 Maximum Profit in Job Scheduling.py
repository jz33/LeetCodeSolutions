...
class Solution:
    def jobScheduling(self, startTime: List[int], endTime: List[int], profit: List[int]) -> int:
        ...
        for st, et, pf in jobs:
            # Find the latest job whose end time ends before current start time
            # j cannot be 0, as all timestamp starts from 1
            # This part can use binary search
            j = len(dp) - 1
            while j > 0 and st < dp[j][0]:
                j -= 1
            
            # Add new profix only if it makes more than last one
            newProfit = dp[j][1] + pf
            if newProfit > dp[-1][1]:
