...
class MyCalendarTwo {
public:
    std::map<int, int> events; // {start : end}
    
    MyCalendarTwo() {}
 
    bool book(int start, int end) {
        // Cannot use Calendar I's binary search method,
        // because the ovelapped intervals cannot be determined.
        // Possible binary search solution maybe have 2 maps, {start : end}, {end : start}
        events[start]++;
        events[end]--;
        ...
        for (auto pair : events) {
            booked += pair.second;
            maxBooked = std::max(maxBooked, booked);
            if (maxBooked > 2) {
                events[start]--;
                events[end]++;
                return false;
