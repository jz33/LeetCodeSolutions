...
class Solution:
    def predictTheWinner(self, nums: List[int]) -> bool:
        ...
        '''
        @cache
        def topdown(left: int, right: int) -> int:
            '''
            @left: left index on nums
            @right: right index on nums
            @return: maximum score on nums[left : right + 1]
            '''
            if left == right:
                return nums[left]

            # The score of current round is the picked card - score of inner rounds
            leftScore = nums[left] - topdown(left + 1, right)
            rightScore = nums[right] - topdown(left, right - 1)
            return max(leftScore, rightScore)
        return topdown(0, len(nums) - 1) >= 0
