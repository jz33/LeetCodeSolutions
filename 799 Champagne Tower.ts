...
to the left and right of it.  When those glasses become full, any excess champagne will fall equally to
...
function champagneTower(poured: number, query_row: number, query_glass: number): number {
    ...
    // currentRow[i] holds the wine.
    // Imagine pouring all wines to top cup, then calculate the wind downward
    let currentRow = [poured];
    for (let rowIndex = 0; rowIndex <= query_row; rowIndex++) {
        const nextRow = new Array(rowIndex + 2).fill(0);
        for (let cupIndex = 0; cupIndex < currentRow.length; cupIndex++) {
            // For each cup in current row, pour the extra wine ( > 1) equally to next 2 cups.
            // Improve: break earlier if all next row cups are empty
            const wine = currentRow[cupIndex];
            if (wine > 1) {
                const extraHalf = (wine - 1) / 2;
                nextRow[cupIndex] += extraHalf;
                nextRow[cupIndex + 1] += extraHalf;
                // Current cup will only hold 1
                currentRow[cupIndex] = 1;
            }
        }
