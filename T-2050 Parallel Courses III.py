...
class Solution:
    def minimumTime(self, n: int, relations: List[List[int]], times: List[int]) -> int:
        ...
        for node in range(n):
            if indegrees[node] == 0:
                queue.append(node)
                maxTimes[node] = times[node]

        # Unlike 1136. Parallel Courses, the iteration is not necessarily level by level
        while queue:
            node = queue.popleft()
            for toNode in graph[node]:
                maxTimes[toNode] = max(maxTimes[toNode], maxTimes[node] + times[toNode])
                indegrees[toNode] -= 1
