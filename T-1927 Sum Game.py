...
class Solution:
    def sumGame(self, num: str) -> bool:
        ...
        BobTotal, BobQuestions = getState(num[: len(num) // 2])
        AliceTotal, AliceQuestions = getState(num[len(num) // 2 :])
        # Alice will try to set the difference even higher by using 9,
        # while bob will try set the difference lower by using 0
        # so each '?' can be count as 4.5
        return BobTotal + BobQuestions * 4.5 != AliceTotal + AliceQuestions * 4.5
