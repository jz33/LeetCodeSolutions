...
adding the event willnot cause a double booking.
...
public:
    ...
    bool book(int start, int end) {
        auto existed = events.lower_bound(start);
        if (existed != events.end() && existed->first < end) {
            // Existed an event who starts before new event's end
            return false;
        }
        if (existed != events.begin() && (--existed)->second > start) {
            // Existed an event who ends after new event's start
            return false;
        }
