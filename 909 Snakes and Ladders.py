...
class Solution:
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        def getBoardValue(n: int) -> int:
            x = (n - 1) // N # row index from bottom-up
            y = (n - 1) % N if x % 2 == 0 else N - 1 - (n - 1) % N
        ...
        while stack:
            ...
            for i in stack:
                ...
                for c in range(i + 1, i + 7):
                    if c not in visited:
                        visited.add(c)
                        
                        # Check if goes to ladder or not
                        t = getBoardValue(c)
                        if t != -1:
                            # Do not add t to visited, as next round
                            # can start from t then go to ladder
                            if t == desti:
                                return steps
                            newStack.append(t)
                        else:
