...
class Cashier {
    ...
    private readonly priceChart: {[key: number] : number} // {product id : price}
    ...
    constructor(n: number, discount: number, products: number[], prices: number[]) {
        ...
        this.priceChart = {}
        const size: number = products.length
        for (let i = 0; i < size; ++i){
            this.priceChart[products[i]] = prices[i]
        }
    }
    ...
    getBill(product: number[], amount: number[]): number {
        ...
        let total : number = 0
        const size: number = product.length
        for (let i = 0; i < size; ++i) {
            total += this.priceChart[product[i]] * amount[i]
        }
        
        if (this.customerCount % this.mod === 0) {
