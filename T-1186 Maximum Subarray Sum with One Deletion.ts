...
function maximumSum(nums: number[]): number {
    let result = nums[0];
    let noDeletionMax = nums[0]; // Same like 'total' in 53. Maximum Subarray
    let atMostOneDeletionMax = Math.max(0, nums[0]);
    for (let i = 1; i < nums.length; i++) {
        [noDeletionMax, atMostOneDeletionMax] = [
            Math.max(nums[i], noDeletionMax + nums[i]),
            // = max(previous no deletion + no curr, previous at most one deletion + curr)
            Math.max(noDeletionMax, atMostOneDeletionMax + nums[i]),
        ];
        result = Math.max(result, noDeletionMax, atMostOneDeletionMax);
    }
    return result;
}
