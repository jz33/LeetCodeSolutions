...
class Solution:
    def predictPartyVictory(self, senate: str) -> str:
        ...
        for index, people in enumerate(senate):
            if people == 'R':
        ...
        while radiantQueue and direQueue:
            radiant = radiantQueue.popleft()
            dire = direQueue.popleft()
            if radiant < dire:
                # R bans D, and put R in next round
                radiantQueue.append(radiant + senateSize)
            else:
                direQueue.append(dire + senateSize)
