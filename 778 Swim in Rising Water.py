...
class Solution:
    ...
    def swimInWater(self, heights: List[List[int]]) -> int:
        ...
        visited = [[None] * colCount for _ in range(rowCount)] # [i][j] : min effort
        visited[0][0] = 0
        heap = [(heights[0][0], 0, 0)] # [(maxHeight, x, y)]
        while heap:
            maxHeight, x, y = heappop(heap)
            if x == rowCount - 1 and y == colCount - 1:
                return maxHeight
            for i, j in (x+1,y), (x-1,y), (x,y+1), (x,y-1):
                if 0 <= i < rowCount and 0 <= j < colCount:
                    newMaxHeight = max(maxHeight, heights[i][j])
                    if visited[i][j] is None or visited[i][j] > newMaxHeight:
                        visited[i][j] = newMaxHeight
                        heappush(heap, (newMaxHeight, i, j))
        return None
