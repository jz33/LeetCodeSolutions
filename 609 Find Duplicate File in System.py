...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        book = collections.defaultdict(list) # {content : full file name}
        for path in paths:
            row = path.split()
            directory = row[0]
            for i in range(1, len(row)):
                file = row[i]
                bi = file.index('(') # first bracket index
                content = file[bi+1 : -1]
                filename = file[:bi]
                book[content].append(directory + '/' + filename)
        
        return [v for v in book.values() if len(v) > 1]
